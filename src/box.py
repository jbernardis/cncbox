...
class box:
    ...
    def render(self, faceType, toolrad):
        if faceType is None:
            return 
        
        if faceType not in [ FACE_TOP, FACE_BOTTOM, FACE_LEFT, FACE_RIGHT, FACE_FRONT, FACE_BACK ]:
            return
            
        self.currentFace = faceType 
        
        return self.faces[faceType].render(toolrad)
